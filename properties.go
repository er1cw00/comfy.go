								...
								if propgroups, ok := pp.([]interface{}); ok {
									if len(propgroups) >= 2 {
										if propname, ok := propgroups[0].(string); ok {
											...
											logger.Debug("TODO - Potential non-string property name")
										}
									} else {
										logger.Debug("TODO - Potential non-slice property group")
									}
								} else {
									logger.Debug("TODO - Potential non-slice property group")
								}
...
func newComboProperty(input_name string, optional bool, input []interface{}, index int) *Property {
	...
	for _, v := range input {
		if s, ok := v.(string); ok {
			...
			logger.Debugf("TODO - Potential non-string combo entry <%s>, %s", reflect.TypeOf(v).Name(), input_name)
		}
	}
	var retv Property = c
