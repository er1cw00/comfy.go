...
func (sm *StatusMessage) UnmarshalJSON(b []byte) error {
	...
	switch sm.Type {
	case "status":
		sm.Data = &MessageDataStatus{}
	case "execution_start":
		sm.Data = &MessageDataExecutionStart{}
	case "execution_cached":
		sm.Data = &MessageDataExecutionCached{}
	case "executing":
		sm.Data = &MessageDataExecuting{}
	case "progress":
		sm.Data = &MessageDataProgress{}
	case "executed":
		// this is a special case because the data type is not always the same
		// so we need to unmarshal it manually
		sm.Data = &MessageDataExecuted{}
	case "execution_interrupted":
		sm.Data = &MessageExecutionInterrupted{}
	case "execution_error":
		sm.Data = &MessageExecutionError{}
	default:
...
type MessageDataExecutionStart struct {
...
type MessageDataExecutionCached struct {
...
type MessageDataProgress struct {
...
type MessageDataExecuted struct {
	Node     int                      `json:"node"`
	Output   map[string]*[]DataOutput `json:"output"`
	PromptID string                   `json:"prompt_id"`
}

func (mde *MessageDataExecuted) UnmarshalJSON(b []byte) error {
	var temp struct {
		Node      string                 `json:"node"`
			...
			for _, i := range val {
				if outmap, ok := i.(map[string]interface{}); ok {
					...
					if ok {
						...
						logger.Warnf("MessageDataExecuted output entry %v unknown type", i)
						continue
					}
...
type MessageExecutionInterrupted struct {
	PromptID string   `json:"prompt_id"`
...
type MessageExecutionError struct {
	PromptID         string                 `json:"prompt_id"`
	Node             string                 `json:"node_id"`
	NodeType         string                 `json:"node_type"`
	Executed         []string               `json:"executed"`
